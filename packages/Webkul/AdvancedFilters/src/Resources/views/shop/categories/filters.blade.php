...
@pushOnce('scripts')
        ...
        <template v-else>
            <div class="panel-side journal-scroll grid max-h-[1320px] min-w-[342px] grid-cols-[1fr] overflow-y-auto overflow-x-hidden max-xl:min-w-[270px] md:max-w-[342px] md:ltr:pr-7 md:rtl:pl-7">
                ...
                <!-- Customer Feedback Section -->
                 @php
                    $showFeedbackForm = core()->getConfigData('general.advancefilter.settings.show_feedback_form');
                @endphp
                <v-customer-feedback v-if="{{ $showFeedbackForm ? 'true' : 'false' }}"></v-customer-feedback>
            </div>
        </template>
    </script>
            ...
            <x-slot:content class="!p-0">
                ...
                <template v-else>
                    ...
                    <div
                        ...
                        v-if="!['ratings', 'availability', 'offers', 'discount', 'popular'].includes(filter.id)"
                    >
                        <div class="relative">
        ...
        <div class="border-t border-gray-300 mt-6 pt-5 px-4 sm:px-6 bg-white rounded-lg shadow-sm">
            ...
            <div class="mb-4">
                <h3 class="text-base font-semibold text-gray-900 mb-1">
                    @lang('advancedfilters::app.shop.feedback.title')
                </h3>
                <p class="text-sm text-gray-500">
                     @lang('advancedfilters::app.shop.feedback.subtitle')
                </p>
            </div>

            <div v-if="submitted" class="text-center py-6">
                <p class="text-sm text-green-700 font-medium">
                    @lang('advancedfilters::app.shop.feedback.thank-you')
                </p>
            </div>
            ...
            <div v-else>
                ...
                <div class="flex gap-3 mb-4">
                    ...
                    >
                        @lang('advancedfilters::app.shop.feedback.yes')
                    </button>
                    <button
                        @click="response = 'no'"
                    ...
                    >
                        @lang('advancedfilters::app.shop.feedback.no')
                    </button>
                </div>
                <div v-if="response" class="mb-4">
                    <textarea
                        ...
                        ? '{{ trans('advancedfilters::app.shop.feedback.placeholder-yes') }}'
                        : '{{ trans('advancedfilters::app.shop.feedback.placeholder-no') }}'"
                ...
                >
                    <span v-if="loading">@lang('advancedfilters::app.shop.feedback.submitting')</span>
                    <span v-else>@lang('advancedfilters::app.shop.feedback.submit')</span>
                </button>
            </div>
        </div>
    </script>
    ...
    <script type='module'>
        app.component('v-filters', {
            ...
            methods: {
                getFilters(){
                    this.$axios.get('{{ route("advancedfilters.api.categories.attributes") }}', {
                        params: {
        ...
        app.component('v-filter-item', {
            ...
            methods: {
                ...
                fetchFilterOptions(replace = true) {
                    const virtualFilters = ['ratings', 'availability', 'offers', 'discount', 'popular'];
                    ...
                    if (this.filter.id === 'category') {
                        url = `{{ route("shop.api.categories.category_options") }}`;
                    } else {
                        url = `{{ route("shop.api.categories.attribute_options", 'attribute_id') }}`.replace('attribute_id', this.filter.id);
                    }
                ...
                async submitFeedback() {
                    ...
                    try {
                        ...
                        if (response.status === 200) {
                           ...
                           throw new Error("@lang('advancedfilters::app.feedback.failed')");
                        }
                    } catch (error) {
                        this.loading = false;
                        let errorMessage = "@lang('advancedfilters::app.feedback.error')";
